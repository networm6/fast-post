 ...
 */
public final class KzPermissions {
     ...
     * 私有化构造函数
     */
    private KzPermissions(Base activity) {
        mActivity = activity;
    }
     ...
     * 设置请求的对象
     */
    public static KzPermissions with(Base activity) {
        return new KzPermissions(activity);
    }

    /**
     * 设置权限组
     */
    public KzPermissions permission(String... permissions) {
     ...
     * 设置权限组
     */
    public KzPermissions permission(List<String> permissions) {
        if (mPermissions == null) {
     ...
     * 被拒绝后继续申请，直到授权或者永久拒绝
     */
    public KzPermissions constantRequest() {
        mConstant = true;
        return this;
    }
