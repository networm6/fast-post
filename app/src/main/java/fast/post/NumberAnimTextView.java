    ...
    private void start() {
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
				...
				public void onAnimationUpdate(ValueAnimator valueAnimator) {
					...
					MainActivity.blurDrawable.setBlur(0+(value.intValue()*10));
					MainActivity.mBlurImage.setImageDrawable(MainActivity.blurDrawable.getBlurDrawable());
				}
			});
        animator.addListener(new AnimatorListenerAdapter() {
				...
				public void onAnimationEnd(Animator animation) {
					...
					//MainActivity.done();
					if(jj==true){
						setNumberString("25", "1");
						jj=false;
					}else{
						setNumberString("1", "25");
						jj=true;
					}
				}
			});
